...
class MiniFWConfig:
    ...
    def load_policy(cls) -> Dict:
        ...
        try:
            ...
            if not os.path.exists(cls.POLICY_PATH):
    ...
    def save_policy(cls, policy: Dict) -> bool:
        ...
        try:
            ...
            # Try to backup existing policy (optional - won't fail if permission denied)
            if os.path.exists(cls.POLICY_PATH):
                try:
                    ...
                    print(f"Warning: Could not create backup file (permission denied), proceeding without backup")
                except Exception as backup_error:
                    print(f"Warning: Could not create backup file: {backup_error}")
            ...
            print(f"Error saving policy: Permission denied - {e}")
            return False
        except Exception as e:
...
class MiniFWFeeds:
    ...
    def read_feed(cls, feed_name: str) -> List[str]:
        ...
        # Create directory if it doesn't exist
        os.makedirs(cls.FEEDS_DIR, exist_ok=True)
        
        if not os.path.exists(file_path):
            # Create empty feed file
            try:
                with open(file_path, 'w') as f:
                    f.write("# MiniFW-AI Feed File\n")
                return []
            except Exception as e:
                print(f"Error creating feed file {feed_name}: {e}")
                return []
    ...
    def write_feed(cls, feed_name: str, entries: List[str]) -> bool:
        ...
        try:
            ...
            # Try to backup existing file (optional - won't fail if permission denied)
            if os.path.exists(file_path):
                try:
                    ...
                    print(f"Warning: Could not create backup for {feed_name} (permission denied), proceeding without backup")
                except Exception as backup_error:
                    print(f"Warning: Could not create backup for {feed_name}: {backup_error}")
            ...
            print(f"Error writing feed {feed_name}: Permission denied - {e}")
            return False
        except Exception as e:
