...
class JsonEnforcerService:
    ...
    def validate_payload(endpoint: str, method: str, payload_raw, version: str = "v1"):
        ...
        try:
            if isinstance(payload_raw, (str, bytes)):
                ...
                return {
                    "valid": False,
                    "message": "Unsupported payload type: Must be JSON string, bytes, or dict",
                }
        ...
        if not payload:
            return {
                "valid": True,
                "message": "Empty or Null payload, skipped schema validation",
            }
        ...
        schema = JsonSchema.objects.filter(
            endpoint=endpoint, method=method_upper, version=version, is_active=True
        ).first()
        ...
        if not schema and version != "v1":
            logger.info(
                f"Schema for {method_upper} {endpoint} v{version} not found. Trying fallback to v1."
            )
            schema = JsonSchema.objects.filter(
                endpoint=endpoint, method=method_upper, version="v1", is_active=True
            ).first()
            fallback_used = True
        ...
        if not schema:
            return {
                "valid": True,
                "message": f"No active schema configured for {method} {endpoint} (version {version}), validation skipped",
            }
        ...
        try:
            ...
            return {
                "valid": False,
                "message": f"Unexpected error during schema processing: {e}",
            }
    ...
    def create_schema(data: dict):
        ...
        if JsonSchema.objects.filter(
            ...
            raise ValueError(
                "Schema with this endpoint, method, and version already exists."
            )
    ...
    def update_schema(schema_id: int, data: dict):
        ...
        if (
            new_endpoint != schema.endpoint
            or new_method != schema.method
            or new_version != schema.version
        ):

            if (
                JsonSchema.objects.filter(
                    endpoint=new_endpoint, method=new_method, version=new_version
                )
                .exclude(pk=schema_id)
                .exists()
            ):
                raise ValueError(
                    "Update failed: Schema with this endpoint, method, and version already exists."
                )
