...
class ValidatePayloadView(APIView):
    ...
    def post(self, request):
        ...
        payload = request.data.get(
            "payload"
        )  # Payload yang dikirim sebagai bagian dari body request
        version = request.data.get("version", "v1")

        if not endpoint or not method or payload is None:
            logger.warning(
                "Missing required fields in ValidatePayloadView POST request."
            )
            return Response(
        ...
        result = JsonEnforcerService.validate_payload(
            endpoint=endpoint, method=method, payload_raw=payload, version=version
        )
        ...
        if result["valid"]:
            ...
            if rollout_mode == "enforce":
                logger.error(
                    f"Schema enforcement block: Endpoint={endpoint}, Method={method}, Error={result['message']}"
                )
                return Response(result, status=response_status)
            else:
                # Jika mode monitor, log warning dan tetap kembalikan 200
                logger.warning(
                    f"Schema violation detected (Monitor mode): Endpoint={endpoint}, Method={method}, Error={result['message']}"
                )
                return Response(result, status=status.HTTP_200_OK)
...
class JsonSchemaListView(APIView):
    ...
    def post(self, request):
        ...
        if (
            not data.get("endpoint")
            or not data.get("method")
            or not data.get("schema_json")
        ):
            return Response(
        ...
        try:
            ...
            logger.info(
                f"Schema created: ID={schema.id}, Endpoint={schema.endpoint} v{schema.version}"
            )
            return Response(
...
class JsonSchemaDetailView(APIView):
    ...
    def put(self, request, pk):
        ...
        try:
            ...
            # Menangkap ValueError dari service (kendala unik atau validasi data)
            logger.warning(
                f"Schema update failed (Conflict/Validation) for ID {pk}: {e}"
            )
            return Response({"error": str(e)}, status=status.HTTP_409_CONFLICT)
        except Exception as e:
    ...
    def delete(self, request, pk):
        ...
        if deleted:
            ...
            return Response(
                {"message": f"Schema ID {pk} deleted successfully"},
