...
class RequestLog(models.Model):
    ...
    session_duration_ms = models.BigIntegerField(
        ...
        blank=True,
        help_text=_(
            "Duration of session in milliseconds (relevant for session behavior anomalies)."
        ),
    )
    ...
    score = models.FloatField(
        help_text=_("Risk score (e.g., 0.0 to 1.0) from the detection engine.")
    )
    ...
    reasons = models.TextField(
        ...
        null=True,
        help_text=_(
            "Detailed reasons or list of features/rules that triggered the action."
        ),
    )
